...
const AdminSalesChart: React.FC<AdminSalesChartProps> = ({ data, title }) => {
            ...
            <div className="flex justify-between items-end h-64 border-l border-b border-gray-700 p-4">
                {data.map((item, index) => (
                    <div key={index} className="flex-1 flex flex-col items-center justify-end h-full relative group">
                        <div className="absolute -top-8 bg-[#2a2a2a] px-2 py-1 text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
